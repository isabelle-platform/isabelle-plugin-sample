      ...
      stages {
        stage('Prepare artifacts (branch)') {
          steps {
            ...
            sh './tools/release.sh --out build/isabelle-plugin-sample-${BRANCH_NAME}-${BUILD_NUMBER}-linux-x86_64.tar.xz'
            /* Copy branch-build-linux to branch-latest-linux */
            sh 'cp build/isabelle-plugin-sample-${BRANCH_NAME}-${BUILD_NUMBER}-linux-x86_64.tar.xz build/isabelle-plugin-sample-${BRANCH_NAME}-latest-linux-x86_64.tar.xz'
          }
        }
        stage('Prepare artifacts (versioned)') {
          ...
          steps {
            ...
            sh 'cp build/isabelle-plugin-sample-${BRANCH_NAME}-latest-linux-x86_64.tar.xz build/versioned_artifacts/isabelle-plugin-sample-${FULL_VERSION}-linux-x86_64.tar.xz'
          }
        }
      }
    }
    stage('Publish artifacts') {
      parallel {
        stage('Publish artifacts (branch)') {
          steps {
            ftpPublisher alwaysPublishFromMaster: true,
                         ...
                         publishers: [
                          [
                            configName: 'isabelle-plugin-sample',
                            transfers:
                              [[
                                asciiMode: false,
                                cleanRemote: false,
                                ...
                                removePrefix: 'build',
                                sourceFiles: 'build/isabelle-plugin-sample-*${BRANCH_NAME}-${BUILD_NUMBER}*.tar.xz'
                              ]],
                          ...
                          [
                            ...
                            transfers:
                              [[
                                ...
                                remoteDirectorySDF: false,
                                removePrefix: 'build',
                                sourceFiles: 'build/isabelle-plugin-sample-*${BRANCH_NAME}-latest*.tar.xz'
                              ]],
                            usePromotionTimestamp: false,
                            useWorkspaceInPromotion: false,
                            verbose: true
                          ...
                          [
                            ...
                            transfers:
                              [[
                                ...
                                sourceFiles: 'build/versioned_artifacts/isabelle-plugin-sample-*.tar.xz'
                              ]],
                            usePromotionTimestamp: false,
                            useWorkspaceInPromotion: false,
                            verbose: true
        ...
        stage('Archive artifacts for Jenkins') {
          steps {
            archiveArtifacts artifacts: 'build/isabelle-plugin-sample-*.tar.xz'
          }
        }
      }
    }
